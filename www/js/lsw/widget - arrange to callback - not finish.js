...
var template_live_status_bar = _.template(jQuery('#template_live_status_bar').remove().text(), null, { variable: "data" }),
	template_result_status_bar = _.template(jQuery('#template_result_status_bar').remove().text(), null, { variable: "data" }),
	template_live_score_cols = _.template(jQuery('#template_live_score_cols').remove().text(), null, { variable: "data" }),
	template_live_new_score_col = _.template(jQuery('#template_live_new_score_col').remove().text(), null, { variable: "data" }),
	template_column_name = _.template(jQuery('#template_column_name').remove().text(), null, { variable: "data" }),
	template_live_row = _.template(jQuery('#template_live_row').remove().text(), null, { variable: "data" }),
	template_schedule_row = _.template(jQuery('#template_schedule_row').remove().text(), null, { variable: "data" }),
	template_result_row = _.template(jQuery('#template_result_row').remove().text(), null, { variable: "data" }),
...
(function ($) {
	...
	$(function () {
		...
		function getFeeds(forceFDays,delayTime,cb){
			if(typeof allowedToGetFeed === 'undefined' || !allowedToGetFeed){
				setTimeout(function(){
					getFeeds(forceFDays,delayTime,cb);
				},2000);
				return;
			}
			...
			$.ajax({
				type: 'POST',
				url: "http://tenisuzivo/wp-content/plugins/live-score-widget/lsw_scraper.php",//?testFileNumber="+testFileNumber/* +"&callback=successAutoCallback" */,
				data: postData,
				dataType: 'jsonp',
		...
		getFeeds(defaultFdays,defaultDelayTime,function(){
			// setTimeout(function(){
				// getFeeds(1,FDAYS_1_INTERVAL,function(){
					// setTimeout(function(){
						// getFeeds(2,FDAYS_2_INTERVAL);
					// },2*60*1000 /*2 min*/);
				// });
			// },1*60*1000 /*1 min*/);
		});
